...
| 系统 | 安装包 |
|---|---|
| macOS Apple Silicon（M1/M2/M3/M4） | [iPix.Preview-mac-arm64.dmg](https://github.com/marswon/iPix/releases/download/v0.21.0-gettoken.8/iPix.Preview-mac-arm64.dmg) |
| macOS Intel x64（macOS 12.0+） | [iPix.Preview-mac-x64.dmg](https://github.com/marswon/iPix/releases/download/v0.21.0-gettoken.8/iPix.Preview-mac-x64.dmg) |
| Windows 10/11 x64 | [iPix.Preview-win-x64.exe](https://github.com/marswon/iPix/releases/download/v0.21.0-gettoken.8/iPix.Preview-win-x64.exe) |

完整发布页：[v0.21.0-gettoken.8](https://github.com/marswon/iPix/releases/tag/v0.21.0-gettoken.8)
...
1. 在 Seedance 2.0 视频节点中切换到 **全能参考**。
2. 连接角色图、参考视频或参考音频；最多可使用 9 张图片、3 段视频和 3 段音频。
3. 只放参考音频不够，Seedance 2.0 还需要至少一张参考图或一段参考视频。
4. 在提示词中明确说明要沿用的角色、动作、镜头或声音特征，再点击生成。
...
请使用 `v0.21.0-gettoken.8` 或更高版本。该版本会把 `1:1 + 1K` 转换成上游要求的像素尺寸。
