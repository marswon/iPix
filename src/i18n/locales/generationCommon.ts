export const zhGenerationCommon = {
  ...
  screenshot: {
    ...
    nodeTitle: '屏幕截图',
    denied: 'macOS 还没给 iPix「屏幕录制」权限，抓不到画面。去「设置 · 通用」里按指引开一下。',
  ...
  observability: {
    ...
    error: {
      ...
      imageRouteDisabled: {
        reason: '中转分组未开通生图路由',
        hint: '这家中转给你的令牌分组没开 OpenAI 生图端点（/v1/images/*）。iPix 已自动改走聊天路由出图；若仍失败，说明分组连聊天出图也没开——去中转控制台给令牌换分组或开通生图，或换一个模型。',
      },
      ...
      assetUploadFailed: {
        reason: '参考图没能送到服务商',
        hint: '服务商要的是一个公网能取到的图片地址，而这张图只在你本机，iPix 只好走免费图床中转（服务商还没被请求到，不扣额度）。这两个图床都在境外，网络或代理没覆盖到它们时就会连不上——最常见的原因是这个，其次才是图床本身临时故障。请先确认代理正常；想彻底跳过这一跳，接一个自带上传通道的服务商（KIE / apimart / 本地 ComfyUI），iPix 会优先走它。',
      },
      assetTooLarge: {
        reason: '这个素材太大，传不上去',
        hint: '本机素材要先换成公网地址才能给服务商用，而这个文件超过了所有可用上传通道的大小上限（HTTP 413），iPix 每条通道都试过了。重试没用——同一个文件每次都会被同一个上限挡回来。请把它压缩或裁短一点再放进来（视频尤其常见），或在「模型接入」接一个上传上限更高的通道。具体是哪个素材、多大，见下方技术详情。',
      },
  ...
  optimizer: {
    ...
    failed: '优化失败',
    optimizedTitle: 'iPix 优化版（高亮=改动）',
    ideaTitle: '说一句想法，iPix 帮你改这条',
    apply: '应用到提示词',
    retry: '重新优化',
    optimizing: '正在优化…',
  ...
  production: {
    ...
    status: {
      submissionUnknown: '提交结果不明，iPix 已停止自动重试',
    ...
    description: {
      ...
      providerStale: 'iPix 仍会查询已有任务，但不会因为等待过久而重复提交。',
      ...
      stalledDraft: '发起它用的 playbook 还没实现，iPix 一个阶段都没能建起来，它不会自己往前走。没有产生任何花费——取消掉，再用支持的 playbook 重新发起。',
      ...
      storyboardReady: '先审阅并确认分镜，确认后 iPix 才会把它落到画布并开始生成并计费。',
    ...
    origin: { nomi: 'iPix', claude: 'Claude Code', codex: 'Codex', cursor: 'Cursor', external: '外部客户端' },
    runPanel: {
      aria: '当前制作状态',
      origin: '来自 {{host}}',
      previewAlt: '最近生成产物',
    ...
    gate: {
      ...
      contractSummary: '确认镜头、模型和硬预算上限后，iPix 才会提交任何付费生成。',
    ...
    reconcile: {
      ...
      notFoundMessage: '确认后 iPix 会记录“未找到”，继续保持暂停；不会自动重新提交或产生新的支出。',
      confirmNotFound: '确认未找到',
      failed: '对账没有生效',
    },
    roughCut: {
      title: '确认粗剪审看结果',
      message: '请先在预览中检查节奏、字幕、声音和素材。确认通过后，iPix 才会进入独立的导出批准。',
      accept: '粗剪通过',
      keepReviewing: '继续审看',
    },
    contract: {
...
export const enGenerationCommon = {
  ...
  comfyuiCancel: {
    ...
    failed: 'iPix could not confirm that ComfyUI cancelled the job. It may still be running locally.',
  },
  spendHostingDisclosure: {
  ...
  screenshot: {
    ...
    denied: 'macOS has not granted iPix Screen Recording permission, so nothing could be captured. Enable it from Settings · General.',
  ...
  observability: {
    ...
    error: {
      ...
      imageRouteDisabled: {
        ...
        hint: 'Your relay token group has no access to the OpenAI image endpoints (/v1/images/*). iPix already retried via the chat route; if it still fails, the group lacks chat image output too — switch the token group or enable image generation in the relay console, or pick another model.',
      },
      ...
      assetUploadFailed: {
        ...
        hint: 'The provider needs a publicly reachable image URL, but this image only exists on your machine, so iPix relayed it through a free image host (the provider was never called, so nothing was charged). Both hosts are overseas and unreachable when your network or proxy does not cover them — that is the most common cause here, ahead of the hosts actually being down. Check your proxy first; to skip this hop for good, connect a provider with its own upload channel (KIE, apimart, or local ComfyUI) and iPix will prefer it.',
      },
      assetTooLarge: {
        reason: 'This asset is too large to upload',
        hint: 'Local assets must first be turned into a publicly reachable URL, and this file exceeds the size limit of every available upload channel (HTTP 413) — iPix tried them all. Retrying will not help: the same file hits the same ceiling every time. Compress or trim it (videos especially) and add it again, or connect a channel with a higher upload limit under Model Access. The technical details below name the asset and its size.',
      },
      server: {
  ...
  optimizer: {
    ...
    optimizedTitle: 'iPix optimized version (highlighted text changed)',
    ideaTitle: 'Tell iPix how you want to improve this prompt',
    ...
    aria: 'Optimize prompt with iPix',
    running: 'Optimizing…',
    optimize: 'Optimize',
  },
  videoToolbar: {
  ...
  production: {
    ...
    status: {
      submissionUnknown: 'Submission result is unknown; iPix stopped automatic retries',
      ...
      running: 'iPix is continuing production',
    ...
    description: {
      ...
      providerStale: 'iPix will keep checking the existing task without resubmitting it because of the delay.',
      completed: 'Every approved stage is complete, and the artifacts remain in this local project.',
      roughCutReady: 'Open Preview to check pacing, captions, and sound. iPix will not overwrite your edit automatically.',
      ...
      stalledDraft: 'The playbook it was started with is not implemented, so iPix never built a single stage and it will never move on its own. Nothing was charged — cancel it and start again with a supported playbook.',
      running: 'Live state appears here. When a provider reports no progress, iPix will not invent a percentage.',
      ...
      storyboardReady: 'Review and confirm the storyboard first. Once you confirm, iPix lays it out and starts generating and billing.',
    ...
    origin: { nomi: 'iPix', claude: 'Claude Code', codex: 'Codex', cursor: 'Cursor', external: 'external client' },
    runPanel: {
    ...
    gate: {
      ...
      contractSummary: 'Review shots, models, and the hard spend limit before iPix submits any paid generation.',
    ...
    checkpoint: {
      ...
      note: 'On confirm, iPix shoots {{count}} shots within the approved {{budget}} budget — this step adds no cost. Reshoot any you do not like; you only pay for that one.',
      defer: 'Not yet',
    ...
    reconcile: {
      ...
      notFoundMessage: 'iPix will record “not found” and remain paused. It will not resubmit or create new spend automatically.',
      confirmNotFound: 'Confirm not found',
      failed: 'Reconciliation did not take effect',
    },
    roughCut: {
      title: 'Confirm rough-cut review',
      message: 'Check pacing, captions, sound, and media in Preview first. After you accept the rough cut, iPix will show a separate export approval.',
    ...
    batch: {
      body: '{{project}} · iPix will make the lead character first for your review, then start filming once you approve.',
