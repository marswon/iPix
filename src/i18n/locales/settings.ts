export const zhSettings = {
  ...
  ai: {
    ...
    upload: {
      ...
      anonymousPromptHint: '未配置 KIE 时仍可临时上传到公共托管；iPix 会先告诉你托管方、有效期和隐私风险。',
    },
    policy: {
      title: '默认模型策略',
      text: '脚本与分镜',
  ...
  automation: {
    ...
    mcp: {
      ...
      hint: '让外部 AI 助手调用 iPix 的工具与工作流',
      manage: '管理连接',
      separationHint: '连接负责“能不能连上”，下面的可信发起方负责“连上后允许做什么”，两种状态分开管理。',
      back: '返回自动化与权限',
      loading: '正在读取 MCP 连接…',
      unavailable: '当前版本无法读取 MCP 连接状态。请重启 iPix 后再试。',
    },
    hosts: {
      title: '可信发起方',
      local: '本机',
      nomi: { name: 'iPix', hint: '本机工作台，负责编辑、批准、采用与导出。' },
  ...
  general: {
    ...
    screenshot: '全局截图热键',
    screenshotHint:
      '默认关。开启后按一下热键就能把屏幕抓进画布——iPix 没在前台也管用（找参考时很顺手）。macOS 需要「屏幕录制」权限。',
    screenshotKey: '热键',
    screenshotKeyTaken: '这组键被别的应用占了，iPix 没抢到。换一组试试。',
    screenshotNeedsPermission:
      'macOS 还没给 iPix「屏幕录制」权限，现在按热键抓不到画面。这项权限没法由应用自己申请，得去系统设置里勾上，勾完要重开 iPix 才生效。',
  ...
  file: {
    title: '文件与保存',
    autoSave: '自动另存生成物',
    autoSaveHint: '默认关。开启后每张生成的图/视频完成时自动复制一份到下面的目录（iPix 内部存储不受影响）。',
    saveTo: '另存到',
...
export const enSettings = {
  ...
  ai: {
    ...
    upload: {
      ...
      channel: {
        ...
        settled: 'Assets no longer pass through a public host. iPix only falls back to public hosting if KIE is unreachable, and asks first.',
      },
      anonymousPrompt: 'Warn me before public temporary hosting',
      anonymousPromptHint: 'If KIE is not configured, iPix can still use a temporary public host, but it will show the host, lifetime, and privacy risk first.',
    },
    policy: {
  ...
  automation: {
    ...
    risk: {
      ...
      continueHint: 'iPix still pauses for a provider change, overspend, or an unknown submission result.',
    ...
    mcp: {
      ...
      hint: 'Let external AI agents use iPix tools and workflows',
      manage: 'Manage connections',
      separationHint: 'Connections control whether an agent can reach iPix. Trusted initiators below control what it may do after connecting.',
      back: 'Back to automation and permissions',
      loading: 'Loading MCP connections…',
      unavailable: 'This version cannot read MCP connection status. Restart iPix and try again.',
    },
    hosts: {
      title: 'Trusted initiators',
      local: 'Local',
      nomi: { name: 'iPix', hint: 'The local workbench owns editing, approval, adoption, and export.' },
  ...
  general: {
    ...
    screenshotHint:
      'Off by default. When on, one keypress grabs the screen into the canvas — even when iPix is not in front (handy while hunting references). macOS needs Screen Recording permission.',
    screenshotKey: 'Hotkey',
    screenshotKeyTaken: 'Another app already owns this key combo, so iPix could not claim it. Try a different one.',
    screenshotNeedsPermission:
      'macOS has not granted iPix Screen Recording permission, so the hotkey cannot capture anything yet. Apps cannot request this permission themselves — enable it in System Settings, then restart iPix.',
  ...
  file: {
    ...
    projectLocationErrorOpenFailed: 'iPix could not open this folder in the file manager.',
    projectLocationErrorUnknown: 'iPix could not change the project location. Nothing was changed.',
  },
}
